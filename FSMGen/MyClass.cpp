////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////

void MyClass::onEnterTestA()
{

}

void MyClass::onExitTestA()
{

}

void MyClass::updateTestA(float dt)
{
	dt;
}

InterfaceResult::Enum MyClass::testTestAOnMyNamedCommand(InterfaceParam* param)
{
	param;
	return InterfaceResult::Unhandled;
}

void MyClass::execTestAOnMyNamedCommand(InterfaceParam* param)
{
	param;
}

void MyClass::onEnterSubstateAA()
{

}

void MyClass::onExitSubstateAA()
{

}

bool MyClass::testSubstateAAToSubstateAB()
{
	return false;
}

void MyClass::execSubstateAAToSubstateAB()
{

}

InterfaceResult::Enum MyClass::testSubstateAAToTestBOnMyNamedCommand(InterfaceParam* param)
{
	param;
	return InterfaceResult::Unhandled;
}

void MyClass::execSubstateAAToTestBOnMyNamedCommand(InterfaceParam* param)
{
	param;
}

void MyClass::onEnterSubstateAB()
{

}

void MyClass::onExitSubstateAB()
{

}

bool MyClass::testSubstateABToTestB()
{
	return false;
}

void MyClass::execSubstateABToTestB()
{

}

void MyClass::onEnterTestB()
{

}

void MyClass::onExitTestB()
{

}

void MyClass::onEnterSubstateBA()
{

}

void MyClass::onExitSubstateBA()
{

}

void MyClass::onEnterSubstateBB()
{

}

void MyClass::onExitSubstateBB()
{

}

